...
from abstract_base.models import OrderStorage, OrderStatus
User = get_user_model()
...
class OrderListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return OrderStorage.objects.filter(user=self.request.user)
...
class OrderDetailView(DetailView):
    ...
    model = OrderStorage
    extra_context = {
        'status_cancelled': OrderStatus.CANCELED
    }

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['detail'] = OrderStorage.objects.filter(pk=self.kwargs['pk'])
        return context


def order_pay_tire(request, pk):
    order = get_object_or_404(OrderStorage, pk=pk)
    order.is_payed = True
    order.save()
